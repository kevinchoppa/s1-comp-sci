...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				...
				if(ans>arr.length){
					System.out.println("numer too big");
				}
					else{
						printsNumber(arr, ans);
					}
					
			}
			else if(choice.equals("change")){
				System.out.println("give me the file name now: ");
				textFile = sc.nextLine();
				arr = fillArray(textFile);
			}
			else if (choice.equals("check")){
				System.out.println("what do u want to check ");
				String ans1 = sc.nextLine();
				System.out.println("there were " + checksArray(arr, ans1) + " many " + ans1 + " within " + textFile);
			}
			else if (choice.equals("common")){
				System.out.println("the most common is " + mostCommon(arr));
				System.out.print("");
			}
	...
	*/
	public static String mostCommon(String[] arr){
		...
		int count = 0;
		for(int i = 0; i<arr.length; i++){
			int temp = checksArray(arr, arr[i]);
			if(temp>count){
				Count = arr[i];
				count = temp;
			}
